...
const applyEditHistoryToProject = (
  ...
  const serialized = history.slice(-MAX_EDIT_HISTORY_ENTRIES).map((entry) => ({
    ...entry,
...
function ReactGeneratorContent() {
  ...
  const [isMobileToolsDropdownOpen, setIsMobileToolsDropdownOpen] =
    useState(false);
  ...
  useEffect(() => {
    if (!isMobileViewport && isMobileEditPanelOpen) {
      setIsMobileEditPanelOpen(false);
    }
  }, [isMobileViewport, isMobileEditPanelOpen]);

  useEffect(() => {
    if (!isMobileViewport && isMobileToolsDropdownOpen) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !target.closest(".mobile-tools-dropdown")
      ) {
        setIsMobileToolsDropdownOpen(false);
      }
    };
  ...
  }, [showExportDropdown, isMobileToolsDropdownOpen]);
  ...
  const hasPendingPublishChanges = (savedProject: SavedProject): boolean => {
    if (!savedProject.publishedUrl) return false;
    const updatedAtTs = toTimestamp(savedProject.updatedAt);
    const publishedAtTs = toTimestamp(savedProject.publishedAt);
    if (updatedAtTs === null || publishedAtTs === null) return false;
...
OVERRIDE: If the user explicitly requested a different behavior (e.g. "just link the PDF", "embed a PDF viewer", "use as background reference"), follow those explicit instructions instead.`;
  ...
  const replaceNthImageSrcInProject = (
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const nextContent = file.content.replace(urlRe, (match, src: string) => {
        if (!sourceCandidates.has(src) || replaced) return match;
        seenUrl++;
          ...
          <header className="flex-shrink-0 border-b border-border-primary bg-bg-secondary/70 backdrop-blur-lg z-10">
            <div className="px-3 sm:px-4 py-3 space-y-2.5">
              <div className="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
                ...
                {isMobileViewport && isMobileToolsDropdownOpen && (
                  <div className="mobile-tools-dropdown rounded-lg border border-border-secondary bg-bg-tertiary/80 p-2 flex flex-col gap-1.5">
                    <button
                      onClick={() => {
                        setTextEditMode((v) => {
                          const next = !v;
                ...
                <div className="flex items-center gap-3 px-3.5 py-3 rounded-xl bg-orange-500/10 border border-orange-500/20 border-dashed">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-orange-300">
                      Click any image in the preview
                    </p>
                    <p className="text-[11px] text-text-muted mt-0.5">
                      Select an image to replace it with your own or generate a
                      new one with AI
                    </p>
                  </div>
                </div>
            ...
            <div className="flex-1 overflow-hidden relative min-h-0 bg-slate-950/95">
              ...
              >
                ...
                >
                  ...
                  {!isMobileViewport && previewMode === "mobile" && (
