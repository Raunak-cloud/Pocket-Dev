...
function SignInContent() {
  ...
  const handleSignIn = async (provider: "google" | "github") => {
    ...
    setLoading(provider);
    try {
      await openSignIn({ redirectUrl: redirect, provider });
    } catch {
      setLoading(null);
    }
  };
  ...
  return (
    <div className="w-full max-w-sm space-y-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border border-border bg-surface hover:bg-surface-hover transition-colors text-sm font-medium text-text-primary disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {loading === "google" ? (
