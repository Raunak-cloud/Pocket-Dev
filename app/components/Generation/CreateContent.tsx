...
interface CreateContentProps {
  ...
  pendingCustomApis: CustomApiConfig[];
  onPendingCustomApisChange: (apis: CustomApiConfig[]) => void;
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-center gap-2 mt-2.5 flex-wrap max-w-2xl">
        ...
        {pendingCustomApis.map((api) => (
          <span
            key={api.id}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/10 border border-blue-500/30 rounded-full"
          >
            <svg className="w-3.5 h-3.5 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
            </svg>
            <span className="text-xs text-blue-200 font-medium">{api.name}</span>
            <button
              type="button"
              onClick={() => onPendingCustomApisChange(pendingCustomApis.filter((a) => a.id !== api.id))}
