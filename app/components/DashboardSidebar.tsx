...
interface DashboardSidebarProps {
  ...
  onBuyTokens?: (tokenType: "app" | "integration") => void;
}
...
export default function DashboardSidebar({
  ...
  const integrationTokens = userData?.integrationTokens || 0;
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-5 border-b border-slate-800/60">
        ...
        >
          ...
          {!isCollapsed && (
            <div>
              <span className="font-bold text-white text-base tracking-tight">Pocket Dev</span>
              <span className="block text-[10px] text-slate-500 font-medium -mt-0.5">Build apps with AI</span>
            </div>
          )}
        </button>
        <button
      ...
      <nav className="flex-1 px-3 pt-5 pb-3 space-y-1.5 overflow-y-auto">
        ...
        >
          <div className={`flex items-center justify-center w-8 h-8 rounded-lg transition ${
            activeSection === "create" ? "bg-blue-500/15" : "bg-slate-800/80 group-hover:bg-slate-700/80"
          }`}>
            <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
            </svg>
          </div>
          {!isCollapsed && <span className="text-sm font-medium">Create App</span>}
        </button>
        ...
        >
          <div className={`flex items-center justify-center w-8 h-8 rounded-lg transition ${
            activeSection === "projects" ? "bg-blue-500/15" : "bg-slate-800/80 group-hover:bg-slate-700/80"
          }`}>
            <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          {!isCollapsed && (
            <>
              <span className="text-sm font-medium flex-1 text-left">My Projects</span>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-800 text-slate-400">{projectCount}</span>
            </>
          )}
        </button>
        ...
        >
          <div className={`flex items-center justify-center w-8 h-8 rounded-lg transition ${
            activeSection === "settings" ? "bg-blue-500/15" : "bg-slate-800/80 group-hover:bg-slate-700/80"
          }`}>
            <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="px-4 pb-4">
          <div className="p-4 bg-gradient-to-br from-slate-800/80 to-slate-900/60 rounded-2xl border border-slate-700/40 space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-500">Token Balance</p>
            </div>
            ...
            <div className="group relative">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs text-slate-400 leading-none">App Tokens</p>
                    <p className="text-[10px] text-slate-600 mt-0.5">2 per project</p>
                  </div>
                </div>
                <span className={`text-lg font-bold tabular-nums ${appTokens > 0 ? "text-blue-400" : "text-red-400"}`}>{appTokens}</span>
              </div>
            </div>
            ...
            <div className="group relative">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs text-slate-400 leading-none">Integration</p>
                    <p className="text-[10px] text-slate-600 mt-0.5">1 per edit / 30 for auth</p>
                  </div>
                </div>
                <span className={`text-lg font-bold tabular-nums ${integrationTokens > 0 ? "text-violet-400" : "text-red-400"}`}>{integrationTokens}</span>
              </div>
            </div>

            {/* Buy Tokens Button */}
            <button
              onClick={() => onBuyTokens?.("app")}
