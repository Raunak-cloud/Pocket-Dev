...
export default function GenerationProgress({
    ...
    return (
      ...
      >
        ...
        <div className="text-left">
          <p className="text-sm font-medium text-white">Building {getAppType()}</p>
  ...
  const stepsPerRow = 4;
  const rows: typeof GENERATION_STEPS[][] = [];
  for (let i = 0; i < GENERATION_STEPS.length; i += stepsPerRow) {
    rows.push(GENERATION_STEPS.slice(i, i + stepsPerRow));
  }
      ...
      <div className="text-center mb-8">
        ...
        <div className="inline-flex items-center gap-4 px-6 py-2.5 bg-slate-900/60 border-2 border-dashed border-slate-700 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-300 font-semibold">In Progress</span>
          </div>
          ...
          {currentStep > 0 && currentStep <= GENERATION_STEPS.length && (
            <div className="mt-6 p-4 bg-blue-500/10 border-2 border-dashed border-blue-500/40 rounded-lg">
              <div className="flex items-center justify-center gap-3">
                <div className="text-2xl">{GENERATION_STEPS[currentStep - 1].icon}</div>
                <div className="text-center">
                  <p className="text-white font-bold text-sm" style={{ fontFamily: 'Comic Sans MS, cursive' }}>
                    → {GENERATION_STEPS[currentStep - 1].label}
                  </p>
